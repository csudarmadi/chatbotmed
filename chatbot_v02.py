...
GROQ_API_KEY = "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"  # Replace with your actual key
...
Jangan berikan informasi di luar yang diminta. Jangan membuat asumsi jika informasi tidak tersedia. Gunakan bahasa yang mudah dipahami pasien dewasa.

⚠️ Jangan gunakan istilah yang tidak lazim secara medis atau tidak dikenal (seperti 'dianamkan'). Jawaban harus akurat, baku, dan tidak mengada-ada.

Jawaban Anda harus profesional, jelas, dan langsung pada poin. Jangan mengulang pertanyaan pengguna.[/INST]"""
...
class Chatbot:
    ...
    def get_info(self, query):
        ...
        is_peringatan = any(keyword in query_norm for keyword in ["peringatan", "larangan", "apa yang harus diperhatikan"])
        ...
        if is_golongan:
            ...
            peringatan = "\n".join(info.get("peringatan", [])) if info.get("peringatan") else "Tidak ada data"
            return f"Peringatan:\n{peringatan}"
...
def get_bot_response(chatbot: Chatbot, user_input: str, priority="kb-first") -> str:
    ...
    try:
        if priority == "llm-first":
            ...
            if not response or "I didn't understand that" in response:
                kb_response = chatbot.get_info(user_input)
                if kb_response:
                    response = kb_response
